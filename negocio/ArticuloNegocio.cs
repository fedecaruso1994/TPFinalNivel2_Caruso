...
namespace negocio
{
    public class ArticuloNegocio
    {
        ...
        public List<Articulo> FiltrarPrecio (decimal desde, decimal hasta)
        {
            AccesoDatos accesoDatos = new AccesoDatos();
            List<Articulo> listaArticulos = new List<Articulo>();
            ...
            try
            {
                string consulta;
                if (hasta == 0) {
                    consulta = @"SELECT AR.Id, AR.Codigo as Código, AR.Nombre, AR.Descripcion , AR.Precio, AR.IdMarca, MA.Descripcion as Marca, AR.IdCategoria, CA.Descripcion as Categoria, AR.ImagenUrl 
                                            FROM ARTICULOS AR INNER JOIN MARCAS MA on AR.IdMarca = MA.Id
                                            INNER JOIN CATEGORIAS CA on AR.IdCategoria = CA.Id 
                                            WHERE AR.Precio >= @desde";
                ...
                else
                {
                    consulta = @"SELECT AR.Id, AR.Codigo as Código, AR.Nombre, AR.Descripcion , AR.Precio, AR.IdMarca, MA.Descripcion as Marca, AR.IdCategoria, CA.Descripcion as Categoria, AR.ImagenUrl 
                                            FROM ARTICULOS AR INNER JOIN MARCAS MA on AR.IdMarca = MA.Id
                                           INNER JOIN CATEGORIAS CA on AR.IdCategoria = CA.Id 
                                           WHERE AR.Precio BETWEEN @desde AND @hasta";
                    accesoDatos.SetearParametro("desde", desde);
